...
class CheXpertTrainer():
    def train(model, dataLoaderTrain, dataLoaderVal, class_names, nnClassCount, trMaxEpoch, PATH, f_or_l, checkpoint, cfg):
        ...
        # Load sample weight ###
        if f_or_l == 'frt':    
            with open('{}weight_1_frt.txt'.format(PATH), 'rb') as f:
                weight_1_raw = np.array(pickle.load(f))
                max_length = max(len(row) for row in weight_1_raw)
                weight_1_pad = np.array([row + [0] * (max_length - len(row)) for row in weight_1_raw])
                weight_1 = torch.from_numpy(weight_1_pad)
                weight_1 = torch.mul(weight_1, round(191027*cfg.train_ratio))
                # weight_1[-1] = weight_1[-1][0:3]
                # print(weight_1[-1])
        else:    
            with open('{}weight_1_lat.txt'.format(PATH), 'rb') as f:
                weight_1_raw = np.array(pickle.load(f))
                max_length = max(len(row) for row in weight_1_raw)
                weight_1_pad = np.array([row + [0] * (max_length - len(row)) for row in weight_1_raw])
                weight_1 = torch.from_numpy(weight_1_pad)
        ...
        for epochID in range(0, trMaxEpoch):
            ...
            losst = CheXpertTrainer.epochTrain(model, dataLoaderTrain, optimizer, nnClassCount, loss, PATH, f_or_l, weight_1) ###
    ...
    def weighted_binary_cross_entropy_16(output, target, weights=None): ###
        if weights is not None:
            weights = weights.repeat_interleave(5)
            weights = weights.reshape(16, 5)
            weights = weights.cuda(non_blocking = True)
            loss = weights * ((target * torch.log(output)) + (1 - target) * torch.log(1 - output))
        else:
            loss = target * torch.log(output) + (1 - target) * torch.log(1 - output)
        return torch.neg(torch.mean(loss))


    def weighted_binary_cross_entropy_last(output, target, weights=None): ###
        if weights is not None:
            weights = weights[:3]
            weights = weights.repeat_interleave(5)
            weights = weights.reshape(3, 5)
            weights = weights.cuda(non_blocking = True)
            loss = weights * ((target * torch.log(output)) + (1 - target) * torch.log(1 - output))
        else:
            loss = target * torch.log(output) + (1 - target) * torch.log(1 - output)
    ...
    def epochTrain(model, dataLoaderTrain, optimizer, nnClassCount, loss, PATH, f_or_l, weight_1): ###
        model.train()
        losstrain = 0
        for batchID, (varInput, target) in enumerate(dataLoaderTrain):
            ...
            if (f_or_l == 'frt' and batchID < 11939) or (f_or_l == 'lat' and batchID < 2024): ###
                lossvalue = CheXpertTrainer.weighted_binary_cross_entropy_16(varOutput, varTarget, weight_1[batchID]) 
            else:
                lossvalue = CheXpertTrainer.weighted_binary_cross_entropy_last(varOutput, varTarget, weight_1[batchID]) 
