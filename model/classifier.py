...
class Classifier(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.backbone = BACKBONES[cfg.backbone](cfg) # densenet121 as default
    ...
    def _init_classifier(self):
        for index, num_class in enumerate(self.cfg.num_classes): # index, value
            if BACKBONES_TYPES[self.cfg.backbone] == 'vgg':
                setattr(
                    ...
                    nn.Conv2d(
                        ...
                        padding=0,
                        bias=True))
            elif BACKBONES_TYPES[self.cfg.backbone] == 'densenet': # default
                setattr( # self.fc_0 = nn.Conv2d(~)
                    self,
                    "fc_" +
                    str(index),
                    nn.Conv2d(
            ...
            if isinstance(classifier, nn.Conv2d):
                classifier.weight.data.normal_(0, 0.01) # weight data from normal distn
    ...
    def _init_bn(self):
        for index, num_class in enumerate(self.cfg.num_classes):
            if BACKBONES_TYPES[self.cfg.backbone] == 'vgg':
                ...
                setattr( # self.bn_0 = nn.BatchNorm2d(~)
                    self,
                    "bn_" +
                    str(index),
                    nn.BatchNorm2d(
    ...
    def _init_attention_map(self):
        if BACKBONES_TYPES[self.cfg.backbone] == 'vgg':
            ...
            setattr( # self.attention_map = AttentionMap(~)
                self,
                "attention_map",
                AttentionMap(
                    self.cfg,
    ...
    def forward(self, x):
        ...
        for index, num_class in enumerate(self.cfg.num_classes):
            ...
            if self.cfg.fc_bn: # True as default
